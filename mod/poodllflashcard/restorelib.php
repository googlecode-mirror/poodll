<?PHP //$Id: restorelib.php,v 1.7 2004/07/01 19:44:55 diml Exp $
    ...
    * @uses $CFG
    */
    function poodllflashcard_restore_mods($mod, $restore) {
        global $CFG;
        ...
        if ($data) {
            ...
            //The structure is equal to the db, so insert the poodllflashcard
            $newid = insert_record ('poodllflashcard', $flashcard);

            //Check the question if exists and is a match or set it to 0
			//Modified this line cos caused errors Justin 20100330
			//if (! $question = get_record('question', 'id', $flashcard->questionid) || $question->qtype != 'match'){
            if (! ($question = get_record('question', 'id', $flashcard->questionid)) || $question->qtype != 'match'){
                $flashcard->questionid = 0;
            }

            //Do some output     
            echo '<ul><li>' . get_string('modulename', 'poodllflashcard') . " \"" . $flashcard->name . "\"<br>";
            backup_flush(300);
            ...
            if ($newid) {
                ...
                $status = $status && poodllflashcard_deckdata_restore_mods ($mod->id, $newid, $info, $restore, $flashcard);

                //Now check if want to restore user data and do it.
                if ($restore->mods['poodllflashcard']->userinfo) {
                    //Restore user decks
                    $status = $status && poodllflashcard_cards_restore_mods($mod->id, $newid, $info, $restore);
                }
                
                //Now restore files
                $status = $status && poodllflashcard_restore_files($mod->id, $newid, $restore);
            } 
            else {
                $status = false;
            }
    ...
    * @uses $CFG 
    */
    function poodllflashcard_deckdata_restore_mods($old_flashcard_id, $new_flashcard_id, $info, $restore, &$flashcard) {
        global $CFG;
        ...
        for($i = 0; $i < sizeof($cards); $i++) {
            ...
            if ($flashcard->questionsmediatype){
                $card->questiontext = preg_replace("/moddata\\/poodllflashcard\\/{$old_flashcard_id}/", "moddata/poodllflashcard/{$new_flashcard_id}", $card->questiontext);
            }

            if ($flashcard->answersmediatype){
                $card->answertext = preg_replace("/moddata\\/poodllflashcard\\/{$old_flashcard_id}/", "moddata/poodllflashcard/{$new_flashcard_id}", $card->answertext);
            }


            //The structure is equal to the db, so insert the card
            $newid = insert_record ('poodllflashcard_deckdata', $card);
            ...
            if ($newid) {
                //We have the newid, update backup_ids
                backup_putid($restore->backup_unique_code, 'poodllflashcard_deckdata', $oldid, $newid);
            } else {
                $status = false;
            }
        }
    ...
    * @uses $CFG 
    */
    function poodllflashcard_cards_restore_mods($old_flashcard_id, $new_flashcard_id, $info, $restore) {
        global $CFG;
        ...
        for($i = 0; $i < sizeof($cards); $i++) {
            ...
            $deckcard = backup_getid($restore->backup_unique_code, 'poodllflashcard_deckdata', $card->entryid);
            if ($deckcard) {
                $card->entryid = $deckcard->new_id;
            }

            //The structure is equal to the db, so insert the card
            $newid = insert_record ('poodllflashcard_card', $card);
            ...
            if ($newid) {
                //We have the newid, update backup_ids
                backup_putid($restore->backup_unique_code, 'poodllflashcard_card', $oldid, $newid);
            } else {
                $status = false;
            }
        }
    ...
    function poodllflashcard_restore_files ($oldflashid, $newflashid, $restore) {
        global $CFG;
        ...
        if ($status) {
            $flashcard_path = $moddata_path."/poodllflashcard";
        ...
        if ($status) {
            $temp_path = $CFG->dataroot."/temp/backup/".$restore->backup_unique_code.
                         "/moddata/poodllflashcard/".$oldflashid;
            //Check it exists
            if (is_dir($temp_path)) {
                $todo = true;
            }
