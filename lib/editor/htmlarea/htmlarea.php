...
HTMLArea.Config = function () {
    ...
    this.toolbar = [
        ...
        [ "justifyleft", "justifycenter", "justifyright", "justifyfull", "separator",
          ...
          "insertimage","insertpoodllwidget", "inserttable",
    ...
    this.btnList = {
        ...
        insertpoodllwidget: [ "Insert/Modify PoodLL Widget", "ed_poodllwidget.gif", false, function(e) {e.execCommand("insertpoodllwidget");} ],
...
HTMLArea.prototype._insertImage = function(image) {
 ...
 //This function works in partner with insertpoodllwidget to get properties out of filter string
 function fetchFilterProperty(filterString,propertyName){
       var retText = "";
       var tIndex = filterString.indexOf(propertyName + "=");
       var eIndex = -1;
       if (tIndex>-1){
               eIndex = filterString.indexOf(",",tIndex);
               if (eIndex == -1){
                       eIndex = filterString.indexOf("}",tIndex);
               }
       }
       ...
       if (eIndex > -1){
               retText = filterString.substring(tIndex + propertyName.length +1,eIndex);
       }
       return retText;
 }

 // Called when the user clicks on "InsertPoodllWidget" button.
 HTMLArea.prototype._insertPoodllWidget = function(media) {

     // Make sure that editor has focus
     this.focusEditor();
     var editor = this;  // for nested functions

       //these are params to init popup window form elements
     var outparam = null;

       //fetch form values from filterstring if selected.
       var sel = this._getSelection();
         var range = this._createRange(sel);
       var txt = HTMLArea.is_ie ? range.text.toString() : sel.toString();
         var len = txt.length;
       var fPath=fetchFilterProperty(txt,"path");
       var fType=fetchFilterProperty(txt,"type");
       var fWidth=fetchFilterProperty(txt,"width");
       var fHeight=fetchFilterProperty(txt,"height");
       var fProtocol=fetchFilterProperty(txt,"protocol");

       //width and height should not be blank in video cases
       //for now we just set them if blank, non video ignores it anyway
       //TO DO: make this more clever Justin 20090707
       if (fWidth == "") fWidth="400";
       if (fHeight == "") fHeight="380";
     ...
     this._popupDialog("<?php
     if(!empty($id) and has_capability('moodle/course:managefiles', get_context_instance(CONTEXT_COURSE, $id))) {
       $retString= "insert_poodllwidget.php?id=$id";
       echo $retString;
     } else {
         echo "insert_poodllwidget_std.php?id=$id";
     }?>", function(param) {
         if (!param) {   // user must have pressed Cancel
             return false;
         }

       var filtertext = param['filtertext'];

     if (HTMLArea.is_ie) {
             range.pasteHTML(filtertext);
         } else {
             // insert the poodll widget filtertext
             editor.insertHTML(filtertext);
         }
         return true;
     }, outparam)
...
HTMLArea.prototype.execCommand = function(cmdID, UI, param) {
    ...
    switch (cmdID) {
        ...
        case "insertpoodllwidget": this._insertPoodllWidget(); break;
    ...
    if (keyEvent) {
        ...
        if (ev.ctrlKey && !ev.altKey) {
            ...
            switch (key) {
                ...
                case 'm': cmd = 'insertpoodllwidget'; break;
